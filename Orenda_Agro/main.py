...
# клас створення нової техніки і додавання її в таблицю
class Add:
...
def add_tech():
    ...
    try:
        new_tech = Add(agro_id, type, name, location, photo, appoint, charact, price)
        Add.set_new(new_tech)
...
def remove_by_id():
    id = int(input("Введіть id по якому хочете видалити рядок: "))
    try:
        connect = get_connection()
        curs = connect.cursor()
...
def remove_by_agro_id():
    agro_id = int(input("Введіть agro_id по якому зочете видалити рядок:"))
    try:
        connect = get_connection()
        curs = connect.cursor()
        print("Підключення успішне")
...
def remove_by_name():
    name = input("Введіть id по якому хочете видалити рядок: ")
    try:
        connect = get_connection()
        curs = connect.cursor()
        print("Підключення успішне")
...
def edit_tech():
    try:
        ...
        curs = connect.cursor()
        print("Підключення успішне")
        # r = curs.execute('SELECT id FROM agro_technics WHERE id = ?', (id, ))
    ...
    try:
        ...
        if len(data) == 0:
            ...
            for row in data:
                ...
                print("Характеристики:", row[7])
                pr = row[8]
                print("Ціна:", pr, "за 1 день / ", ((pr * 7) - pr), "за 7 днів /", ((pr * 30) - (pr * 7)), "за 30 днів",
                      end="\n\n")
            print("Дані зчитано успішно\n"
                  "--------------------------")
        curs.close()
